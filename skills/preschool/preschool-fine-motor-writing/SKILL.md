...
description: "学前控笔与书写准备 Skill把控笔做成孩子愿意参与、家长能照着做、过程可观察的入学准备任务。重点不是提前小学化，而是在真实生活、绘本、游戏和表达里建立安全感、兴趣和基础能力。 Workflow: preschool_fine_motor_writing.run."
version: "0.17.0"
author: zhongwei
license: MIT
...
metadata:
  hermes:
    ...
    legacy_slugs: []
    install_aliases: ["preschool-fine-motor-writing"]
    stages: ["preschool"]
    subjects: ["学习能力"]
    abilities: ["控笔", "书写准备"]
    scenarios: ["入学准备", "每日练习"]
    ...
    requires_tools: ["context.load", "entitlement.check", "workflow.create", "practice.generate_items", "practice.grade_answers", "activity.generate_play_task", "parent_guidance.generate", "memory.write"]
    requires_data: ["孩子年龄/阶段", "启蒙主题或入学准备目标", "可用时间", "家长是否陪伴", "可用材料", "孩子最近状态或兴趣点"]
---

# 学前控笔与书写准备 Skill

学前控笔与书写准备 Skill把控笔做成孩子愿意参与、家长能照着做、过程可观察的入学准备任务。重点不是提前小学化，而是在真实生活、绘本、游戏和表达里建立安全感、兴趣和基础能力。
...
- 5 岁孩子想做控笔，今晚只有 10 分钟，帮我设计一个亲子小游戏。
- 孩子还有三个月上小学，坐不住也不太会表达需求，帮我做幼小衔接家庭准备。
...
- Install Aliases: `preschool-fine-motor-writing`
- Stages: `preschool`
- Subjects: `学习能力`
- Abilities: `控笔`, `书写准备`
...
- Requires Tools: `context.load`, `entitlement.check`, `workflow.create`, `practice.generate_items`, `practice.grade_answers`, `activity.generate_play_task`, `parent_guidance.generate`, `memory.write`
