---
name: "senior-politics-tongbian-textbook-sync"
description: "把统编版高中政治的年级、册别、单元和学习场景，转成预习、同步练习、错题追练、单元复习和考前巩固等可直接执行的学习任务。让道法/政治同步学习从背模板变成会读材料、会表达观点。 Workflow: senior_politics_tongbian_textbook_sync.run."
version: "0.14.0"
author: zhongwei
license: MIT
...
metadata:
  hermes:
    tags: ["education", "senior", "政治", "教材同步", "材料分析", "课前预习", "课后作业", "同步巩固", "单元复习", "错题巩固", "期中期末复习", "高考复习"]
    source: hermes-edu-skills
    workflow: "senior_politics_tongbian_textbook_sync.run"
    category: "textbook-sync"
    category_aliases: ["教材同步", "同步教材", "textbook", "textbook-sync"]
    legacy_slugs: []
    install_aliases: ["senior-politics-tongbian-textbook-sync"]
    stages: ["senior"]
    subjects: ["政治"]
    abilities: ["教材同步", "材料分析"]
...
把统编版高中政治的年级、册别、单元和学习场景，转成预习、同步练习、错题追练、单元复习和考前巩固等可直接执行的学习任务。让道法/政治同步学习从背模板变成会读材料、会表达观点。
...
- 定位本次学习目标和前置基础，重点关注核心概念、材料情境、观点组织、现实联系和规范表达。
- 如果是课前预习，先确认本课概念和生活/社会情境，再让学生用概念解释材料。
- 如果是作业或巩固，把概念辨析、材料关键词提取、观点组织和规范短答组合练习。
- 如果是单元/期末复习，围绕概念网络、材料题结构、现实案例和表达层次组织复习；如果是错题巩固，区分概念混淆、材料脱节、观点空泛和表达没有层次。
...
- 统编版高中政治，高一必修一专题一，帮我做 15 分钟课前预习。
- 统编版高中政治同步，高一必修一专题一，孩子最近知道概念但不会结合材料展开，答案像背模板，生成一组错题追练。
- 按统编版高中政治做单元复习，年级是高一，册别是必修一，请给我知识点、易错点和原创练习。
...
意图：
- `senior_politics_tongbian_textbook_sync`
- `textbook_sync`
- `课前预习`
- `课后作业`
- `同步巩固`
...
- Install Aliases: `senior-politics-tongbian-textbook-sync`
- Stages: `senior`
- Subjects: `政治`
- Abilities: `教材同步`, `材料分析`
