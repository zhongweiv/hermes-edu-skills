---
name: "senior-geography-xj-textbook-sync"
description: "把湘教版高中地理的年级、册别、单元和学习场景，转成预习、同步练习、错题追练、单元复习和考前巩固等可直接执行的学习任务。让地理同步学习从背区域结论变成会读图、找证据、解释为什么。 Workflow: senior_geography_xj_textbook_sync.run."
version: "0.14.0"
author: zhongwei
license: MIT
...
metadata:
  hermes:
    ...
    workflow: "senior_geography_xj_textbook_sync.run"
    category: "textbook-sync"
    category_aliases: ["教材同步", "同步教材", "textbook", "textbook-sync"]
    legacy_slugs: []
    install_aliases: ["senior-geography-xj-textbook-sync"]
    stages: ["senior"]
    subjects: ["地理"]
    abilities: ["教材同步", "图表分析"]
...
把湘教版高中地理的年级、册别、单元和学习场景，转成预习、同步练习、错题追练、单元复习和考前巩固等可直接执行的学习任务。让地理同步学习从背区域结论变成会读图、找证据、解释为什么。
...
- 湘教版高中地理，高一必修一专题一，帮我做 15 分钟课前预习。
- 湘教版高中地理同步，高一必修一专题一，孩子最近地图题容易定位错，综合题成因链写不完整，生成一组错题追练。
- 按湘教版高中地理做单元复习，年级是高一，册别是必修一，请给我知识点、易错点和原创练习。
...
意图：
- `senior_geography_xj_textbook_sync`
- `textbook_sync`
- `课前预习`
- `课后作业`
- `同步巩固`
...
- Install Aliases: `senior-geography-xj-textbook-sync`
- Stages: `senior`
- Subjects: `地理`
- Abilities: `教材同步`, `图表分析`
