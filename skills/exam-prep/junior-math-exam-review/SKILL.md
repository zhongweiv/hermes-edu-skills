---
name: "junior-math-exam-review"
description: "把初中数学考试备考从“泛泛刷题”变成诊断、提分优先级、限时训练、错题复盘和下次复测的闭环，帮助用户知道现在最该抓什么、每天怎么练、怎么判断有没有进步。 Workflow: junior_math_exam_review.run."
version: "0.17.0"
author: zhongwei
license: MIT
...
metadata:
  hermes:
    tags: ["education", "junior", "数学", "考试复习", "查漏补缺", "中考复习", "考前冲刺"]
    source: hermes-edu-skills
    workflow: "junior_math_exam_review.run"
    category: "exam-prep"
    category_aliases: ["考试备考", "备考复习", "exam", "exam-prep"]
    legacy_slugs: []
    install_aliases: ["junior-math-exam-review"]
    stages: ["junior"]
    subjects: ["数学"]
    abilities: ["考试复习", "查漏补缺"]
    scenarios: ["中考复习", "考前冲刺"]
...
把初中数学考试备考从“泛泛刷题”变成诊断、提分优先级、限时训练、错题复盘和下次复测的闭环，帮助用户知道现在最该抓什么、每天怎么练、怎么判断有没有进步。
...
- 先区分基础题丢分、方法不会、模型选错和时间分配问题，给出 1-3 个最优先提分点。
- 按概念、计算、函数/几何模型、综合题拆解和压轴题取分策略设计复习路径，区分保分项、突破项和放弃/降权项。
- 按基础保分、题型突破、综合拆步和限时套卷分层训练，每个任务都标注时长、题量、难度和完成标准。
- 把错题按概念、计算、审题、模型、步骤表达和时间压力分类，把每次训练转成错因卡片、同类追练和下次复测时间。
...
- 初中数学考试，我现在是七年级，还有14 天，每天 45 分钟，帮我做提分计划。
- 初中数学考试复习，最近函数和几何综合题容易卡住，基础计算偶尔丢分，请先诊断再安排 7 天专项训练。
- 下面是我最近 5 道错题，请按初中数学考试要求做错因分类、同类追练和复测安排。
...
- Install Aliases: `junior-math-exam-review`
- Stages: `junior`
- Subjects: `数学`
- Abilities: `考试复习`, `查漏补缺`
