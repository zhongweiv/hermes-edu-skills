---
name: "junior-biology-exam-review"
description: "把初中生物考试备考从“泛泛刷题”变成诊断、提分优先级、限时训练、错题复盘和下次复测的闭环，帮助用户知道现在最该抓什么、每天怎么练、怎么判断有没有进步。 Workflow: junior_biology_exam_review.run."
version: "0.13.0"
author: zhongwei
license: MIT
...
metadata:
  hermes:
    tags: ["education", "junior", "生物", "考试复习", "查漏补缺", "中考复习", "考前冲刺"]
    source: hermes-edu-skills
    workflow: "junior_biology_exam_review.run"
    category: "exam-prep"
    category_aliases: ["考试备考", "备考复习", "exam", "exam-prep"]
    legacy_slugs: []
    install_aliases: ["junior-biology-exam-review"]
    stages: ["junior"]
    subjects: ["生物"]
    abilities: ["考试复习", "查漏补缺"]
    scenarios: ["中考复习", "考前冲刺"]
...
把初中生物考试备考从“泛泛刷题”变成诊断、提分优先级、限时训练、错题复盘和下次复测的闭环，帮助用户知道现在最该抓什么、每天怎么练、怎么判断有没有进步。
...
- 先看概念混淆、图表阅读、实验变量和表达准确性，给出 1-3 个最优先提分点。
- 按核心概念、结构功能、实验变量、图表信息和规范表达设计复习路径，区分保分项、突破项和放弃/降权项。
- 按概念辨析、图表题、实验设计、材料分析和短答表达训练，每个任务都标注时长、题量、难度和完成标准。
- 把错题按名词混淆、结构功能、变量判断、图表遗漏和表达不规范分类，把每次训练转成错因卡片、同类追练和下次复测时间。
...
- 初中生物考试，我现在是七年级，还有14 天，每天 45 分钟，帮我做提分计划。
- 初中生物考试复习，最近图表题信息提取不完整，实验变量判断不稳，请先诊断再安排 7 天专项训练。
- 下面是我最近 5 道错题，请按初中生物考试要求做错因分类、同类追练和复测安排。
...
- Install Aliases: `junior-biology-exam-review`
- Stages: `junior`
- Subjects: `生物`
- Abilities: `考试复习`, `查漏补缺`
