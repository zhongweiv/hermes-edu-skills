---
name: "senior-geography-exam-review"
description: "把高中地理考试备考从“泛泛刷题”变成诊断、提分优先级、限时训练、错题复盘和下次复测的闭环，帮助用户知道现在最该抓什么、每天怎么练、怎么判断有没有进步。 Workflow: senior_geography_exam_review.run."
version: "0.13.0"
author: zhongwei
license: MIT
...
metadata:
  hermes:
    tags: ["education", "senior", "地理", "考试复习", "查漏补缺", "高考复习", "考前冲刺"]
    source: hermes-edu-skills
    workflow: "senior_geography_exam_review.run"
    category: "exam-prep"
    category_aliases: ["考试备考", "备考复习", "exam", "exam-prep"]
    legacy_slugs: []
    install_aliases: ["senior-geography-exam-review"]
    stages: ["senior"]
    subjects: ["地理"]
    abilities: ["考试复习", "查漏补缺"]
    scenarios: ["高考复习", "考前冲刺"]
...
把高中地理考试备考从“泛泛刷题”变成诊断、提分优先级、限时训练、错题复盘和下次复测的闭环，帮助用户知道现在最该抓什么、每天怎么练、怎么判断有没有进步。
...
- 先看地图定位、图表信息提取、区域特征记忆和成因解释是否断裂，给出 1-3 个最优先提分点。
- 按地图判读、区域定位、图表信息、成因链条和人地关系表达设计复习路径，区分保分项、突破项和放弃/降权项。
- 按读图定位、材料提取、区域比较、成因分析和综合题表达分层训练，每个任务都标注时长、题量、难度和完成标准。
- 把错题按地图读错、区域特征混淆、因果链不完整和材料遗漏分类，把每次训练转成错因卡片、同类追练和下次复测时间。
...
- 高中地理考试，我现在是高一，还有14 天，每天 45 分钟，帮我做提分计划。
- 高中地理考试复习，最近地图题容易定位错，综合题成因总写不完整，请先诊断再安排 7 天专项训练。
- 下面是我最近 5 道错题，请按高中地理考试要求做错因分类、同类追练和复测安排。
...
- Install Aliases: `senior-geography-exam-review`
- Stages: `senior`
- Subjects: `地理`
- Abilities: `考试复习`, `查漏补缺`
