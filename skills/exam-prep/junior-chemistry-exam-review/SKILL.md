---
name: "junior-chemistry-exam-review"
description: "把初中化学考试备考从“泛泛刷题”变成诊断、提分优先级、限时训练、错题复盘和下次复测的闭环，帮助用户知道现在最该抓什么、每天怎么练、怎么判断有没有进步。 Workflow: junior_chemistry_exam_review.run."
version: "0.17.0"
author: zhongwei
license: MIT
...
metadata:
  hermes:
    tags: ["education", "junior", "化学", "考试复习", "查漏补缺", "中考复习", "考前冲刺"]
    source: hermes-edu-skills
    workflow: "junior_chemistry_exam_review.run"
    category: "exam-prep"
    category_aliases: ["考试备考", "备考复习", "exam", "exam-prep"]
    legacy_slugs: []
    install_aliases: ["junior-chemistry-exam-review"]
    stages: ["junior"]
    subjects: ["化学"]
    abilities: ["考试复习", "查漏补缺"]
    scenarios: ["中考复习", "考前冲刺"]
...
把初中化学考试备考从“泛泛刷题”变成诊断、提分优先级、限时训练、错题复盘和下次复测的闭环，帮助用户知道现在最该抓什么、每天怎么练、怎么判断有没有进步。
...
- 先看方程式、实验现象、推断路径和计算步骤的失分来源，给出 1-3 个最优先提分点。
- 按概念辨析、化学用语、实验探究、物质推断和计算设计复习路径，区分保分项、突破项和放弃/降权项。
- 按化学用语、实验分析、推断流程、计算和综合题分层训练，每个任务都标注时长、题量、难度和完成标准。
- 把错题按符号错误、现象描述、微观解释、条件遗漏和计算步骤分类，把每次训练转成错因卡片、同类追练和下次复测时间。
...
- 初中化学考试，我现在是七年级，还有14 天，每天 45 分钟，帮我做提分计划。
- 初中化学考试复习，最近实验题现象写不准，推断题条件整理容易乱，请先诊断再安排 7 天专项训练。
- 下面是我最近 5 道错题，请按初中化学考试要求做错因分类、同类追练和复测安排。
...
- Install Aliases: `junior-chemistry-exam-review`
- Stages: `junior`
- Subjects: `化学`
- Abilities: `考试复习`, `查漏补缺`
