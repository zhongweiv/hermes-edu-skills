---
name: "senior-history-exam-review"
description: "把高中历史考试备考从“泛泛刷题”变成诊断、提分优先级、限时训练、错题复盘和下次复测的闭环，帮助用户知道现在最该抓什么、每天怎么练、怎么判断有没有进步。 Workflow: senior_history_exam_review.run."
version: "0.13.0"
author: zhongwei
license: MIT
...
metadata:
  hermes:
    tags: ["education", "senior", "历史", "考试复习", "查漏补缺", "高考复习", "考前冲刺"]
...
把高中历史考试备考从“泛泛刷题”变成诊断、提分优先级、限时训练、错题复盘和下次复测的闭环，帮助用户知道现在最该抓什么、每天怎么练、怎么判断有没有进步。
...
- 先看时间顺序、史实记忆、材料提取和因果解释的失分来源，给出 1-3 个最优先提分点。
- 按时间线、史料信息、事件因果、阶段特征和观点表达设计复习路径，区分保分项、突破项和放弃/降权项。
- 按时间线复现、材料题提取、因果比较和观点短答训练，每个任务都标注时长、题量、难度和完成标准。
- 把错题按年代混淆、材料漏读、因果链缺失和表达空泛分类，把每次训练转成错因卡片、同类追练和下次复测时间。
...
- 高中历史考试，我现在是高一，还有14 天，每天 45 分钟，帮我做提分计划。
- 高中历史考试复习，最近材料题总抓不到关键词，事件影响也写得很散，请先诊断再安排 7 天专项训练。
- 下面是我最近 5 道错题，请按高中历史考试要求做错因分类、同类追练和复测安排。
