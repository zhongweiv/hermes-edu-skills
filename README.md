...
```bash
# 一键安装并启用全部 188 个可用 Skill。安装后继续用 hermes / hermes chat 即可
...
如果你的 Hermes 安装使用了非默认配置，再指定配置文件：
...
```bash
# 启动原生 Hermes 会话。安装后通常直接 hermes 即可
npx hermes-edu-skills chat
...
| 目标 | 命令 |
| --- | --- |
| Hermes 单个 Skill | `npx --yes hermes-edu-skills install agent-study-plan` |
...
- Hermes Agent：推荐使用 `npx --yes hermes-edu-skills install` 一键安装并启用。
